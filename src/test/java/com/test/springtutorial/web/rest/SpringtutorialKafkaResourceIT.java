...
import com.test.springtutorial.config.KafkaProperties;
...
class SpringtutorialKafkaResourceIT {
    ...
    void setup() {
        ...
        SpringtutorialKafkaResource kafkaResource = new SpringtutorialKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/springtutorial-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/springtutorial-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
